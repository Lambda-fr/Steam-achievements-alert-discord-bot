...
async function updateAllUsersAchievements(appData) {
  ...
  for (const gameToAdd of gamesToAdd) {
    ...
    if (game) {
      console.log(`Game ${game.id} (${gameToAdd.appid}) added`);
...
async function processAndDisplayNewAchievements(appData) {
  ...
  console.log(`New achievements to process: ${new_achievements.length}`);
  try {
    for (const newA of new_achievements) {
      for (const guild_id of newA[0].guilds) {
        const guild = appData.guilds.find(g => g.id === guild_id);
        if (typeof guild === 'undefined' || typeof guild.achievements_channel === 'undefined' || typeof guild.achievements_channel_id === 'undefined' || !guild.display_new_achievements_enabled) {
          continue;
        }
        const game = appData.games.get(newA[1].gameId);
        if (guild.display_all_achievements || game.guilds.includes(guild_id)) {
          try {
    ...
    console.error("Error processing new achievements:", err);
  }
